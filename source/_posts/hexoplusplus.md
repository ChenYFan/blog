...
index_img: >-
  https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612929292000.jpg
banner_img: >-
  https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612929292000.jpg
abbrlink: 348e7d8a
---
...
<!--more-->
先前，我曾使用白嫖的Euserv搭建的Typecho，也是用过wordpress.com白嫖的wordpress，但两个都不符合我对速度和可用性的追求，一个连CloudFlare能不能连上都是问题，另一个中国支持贼差【虽然可以用万能Worker可以替换加解决,但是就是不想用啊】。免空的选择又难以择手![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/194.jpg)，弄来弄去还是用回Hexo。

但是Hexo就是有一点不爽，每次使用的时候就必须要在本地进行构建静态网页，然后上传到GithubPage。后来实现了集成部署【没想到折腾了很长时间的集成部署最后用到这里了】，方便了不少，直接在Github上面改源代码。但相较于Typecho和Wordpress，没有后台的写作总感觉有点难受![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896e9710dfd5.jpg)，每次更改源代码都要上Github![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896ece2ab57a.jpg)，在国内这种大环境下总是不太好使的。
...
这篇文章,就是详细讲解我如何把这个梦想变成现实.具体步骤很多,请慢慢咀嚼![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896ece2ab57a.jpg)
...
我们第一个排除heroku，冷启动唤醒需要10s，并且无法绑定域名【这里其实也可用worker反代（bushi】。目光看向worker和vercel，又有一个新问题出来，自定义配置存哪？![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896e6ec1d528.jpg)

存变量里当然是个好主意，但是很难修改。外部存储也不是什么大问题，mongodb、firebase、~~Leancloud~~都可以上手，但我个人终究不喜欢为了查询而发送子请求。![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/194.jpg)
...
可以，这样我们就简单实现了一个无服务器函数![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5c53d1904dcb2.gif)
...
> 这地方我偷懒了,本来应该用`then`来获取`promise`的值,但是个人习惯了`await`嵌套写法,所以这地方写的其实不标准,轻喷![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/285.jpg)
...
当然显然是下面的好写,但我习惯测试方便都用上面的![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/stick_18.png)
...
![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612929973000.png)
...
后端API本质上是一个中继,简单如我![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/7EYyq1TcBKa3eQ2.jpg)
...
能,当然能,但是问题是如果在登录页面还要读KV,那被打了怎么办![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/67.jpg)

况且,在粘贴代码完后到设置界面,中间有一段时间,万一有个人搞你咋办呢.![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/5.jpg)

所以HPP学习Twikoo进行强鉴权,在保证不被盗取的情况下还能减少KV读取量,岂不美哉![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/stick_64.png)
...
> 这个地方在群里我一直和[2X](https://noionion.top/)吵架,因为我觉得此处用广搜比较好,然后我一直想写BFS,结果写着写着就成DFS了,你甚至现在还能看到一个叫`fetch_bfs`的函数![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/BnTMX35EPxleVmA.jpg)
...
![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612936420000.png)
...
![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612937339000.jpg)


~~其实我本来想这样的~~

![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612937329000.jpg)
...
![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612938453000.gif)
...
然后手机端炸了![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5c53cee8422fc.jpg)

万万没想到,safari会将post请求给缓存了![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896ece29a8e0.jpg)

缓存也就罢了,结果ajax连`onreadystatechange`都缓存了不返回,然后接下去的函数全炸了![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896ece2a019f.jpg)
...
这怕是所有Worker程序里面第一个实现自动更新的程序了~~【所以我最近发包很快啊】~~![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612940443000.gif)
...
![](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612941346000.png)
...
但我懒![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896e8a408253.jpg)

其实你也可以用其它什么能定时访问的带上cookie访问`/hpp/admin/api/update`就行![](https://npm.elemecdn.com/chenyfan-oss@1.1.8/5896ece29a8e0.jpg)

## 功能实现 - 文章管理&草稿

其实就是上传文件![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/92.jpg)
...
其实还是上传文件![](https://npm.elemecdn.com/chenyfan-oss@1.1.11/162.jpg)
...
!['说说发布页面'](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612945005000.png)

!['用户查看界面'](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612945225000.jpg)

甚至支持自定义主题：

!['由2X开发的说说主题'](https://npm.elemecdn.com/chenyfan-cdn@2.0.0/img/hpp_upload/1612945296000.jpg)
