...
~~非营利性~~恰AFF饭的私人博客,但是很有限度,因为太穷了

### 方式：

以[Hexo](https://hexo.io/zh-cn/)为框架,主题为[NexT 5.1.4](http://theme-next.iissnan.com), ~~建立在Coding Pages上(原本想直接扔在Github上，但由于`不可告人`的秘密,速度哦出奇的慢)~~ 在2020年初发现Coding转型升级了，接着使用需要实名认证，无奈之下迁回Github，不过加了个CloudFlareCDN，速度在压力测试下 ~~大约稳定在80kb/s~~ ~~很抱歉通过 [站内自选CDN](/2020/03/11/如何不出CloudFlare随便搞一搞将托管在其上的网站速度提升20倍/#more) 现在已经比Github打开还要快了,目前1.7Mb/s的凶狠速度是可以保证的，ip可以手动切换CloudFlare CDNIP以获得更好速度。~~ CloudFlare+JsdelivrCDN，前后端分离。

内部统计使用[百度统计](https://tongji.baidu.com),评论系统为~~Valine~~ ~~Artalk~~ Twikoo..!。

目前本站并没有被 `墙` ,未来也尽可能保证不被墙.因为本站就是一个分享互联网知识的博客小站点,并没有包含任何反动的内容!

![](https://npm.elemecdn.com/chenyfan-os@0.0.0-r14/ChenGlobalBlogNetwork.png)
