...
const handle = async function (req) {
    ...
    if (pathname.match(/\/p\/ssti/g)){
        return fetch(`https://sensitive.215213344.xyz/${pathname.split('/p/ssti')[1].replace('.html','')}.md`)
        .then(res=>res.text())
        .then(res=>{
            return new Response(marked.parse(res), { headers: { "content-type": "text/html; charset=utf-8" } })
        })
    }
    try {
